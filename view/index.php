...
// 2. GARANTÍA DE DATOS PARA NOTIFICACIONES Y MENÚS
// Forzamos que siempre haya una sucursal y almacén (mínimo 1) para que los contadores funcionen
if (!isset($_SESSION['S_IDSUCURSAL']) || empty($_SESSION['S_IDSUCURSAL'])) { $_SESSION['S_IDSUCURSAL'] = 1; }
if (!isset($_SESSION['S_IDALMACEN']) || empty($_SESSION['S_IDALMACEN'])) { $_SESSION['S_IDALMACEN'] = 1; }
...
// Si el usuario es 'admin', traemos los menús del Rol 1 (Administrador) directamente.
if ($_SESSION['S_USUARIO'] == 'admin') {
    $datos = $rol->get_menu_x_rol(1); 
} else {
    $datos = $rol->get_menu_x_rol($_SESSION['S_ROL']);
}

// 4. LÓGICA DE VISTA INICIAL
...
$rutaFinal = ($vistaInicioRuta !== null) ? $vistaInicioRuta : 'dashboard/mantenimiento_dashboard.php';

echo "<script>
    window.onload = function() {
        if(typeof cargar_contenido === 'function') {
            cargar_contenido('contenido_principal', '$rutaFinal');
        }
    }
</script>";
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="../utilitarios/summernote/font/summernote.woff2" />
    <link rel="stylesheet" type="text/css" href="../utilitarios/summernote/font/summernote.woff" />
    <link rel="stylesheet" type="text/css" href="../utilitarios/summernote/font/summernote.ttf" />
    ...
    <link rel="stylesheet" type="text/css" href="../utilitarios/summernote/summernote-lite.min.css" />
...
<body class="sidebar-mini layout-fixed">
    <div class="wrapper">
                ...
                <nav class="mt-2">
                    <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
                        ...
                        <input type="text" value="<?php echo $_SESSION['S_ROL'];  ?>" id="text_idrol" hidden disabled>
                         <input type="text" value="<?php echo $_SESSION['S_IDSUCURSAL'];  ?>" id="text_idsucurs" hidden  disabled>
                         <input type="text" value="<?php echo $_SESSION['S_IDALMACEN'];  ?>" id="text_idalmacen" hidden  disabled>
                        ...
                        ?>
                          ...
                          <?php
                            ...
                            $haySubmenu = false;
                            foreach ($datos as $row) {
                                if ($row["grupo_id"] == "19" && $row["mend_permi"] == "Si") {
                                    $haySubmenu = true;
                                    break; 
                                }
                            }
                        ...
                        <?php
                         ...
                         <?php
                            $haySubmenu = false;
                            foreach ($datos as $row) {
                                if ($row["grupo_id"] == "4" && $row["mend_permi"] == "Si") {
                                    $haySubmenu = true;
                                    break; 
                                }
                            }
                         ...
                         <?php
                           $mostradoCopia = false;
                           foreach ($datos as $row) {
            // Grupo 18 detectado en tu respaldo de tecnophone.shop
                               if ($row["grupo_id"] == "18" && $row["mend_permi"] == "Si" && !$mostradoCopia) {
                                   $mostradoCopia = true;
                     ?>
                <li class="nav-item">
                    <a href="#" class="nav-link">
                    ...
                    <ul class="nav nav-treeview">
                        <?php
                        foreach ($datos as $rowSub) {
                            if ($rowSub["grupo_id"] == "18" && $rowSub["mend_permi"] == "Si") {
                                ?>
                                <li class="nav-item">
                                    <a href="#" class="nav-link" onclick="cargar_contenido('contenido_principal','<?php echo $rowSub['men_ruta'];?>')">
                                        <i class="far fa-circle nav-icon"></i>
                                        <p><?php echo $rowSub["men_vista"]; ?></p>
                                    </a>
                                </li>
                                <?php
                            }
        ...
        <footer class="main-footer">
            ...
            <!-- Default to the left -->
            <strong>Copyright &copy; 2024 <a href="#">SerTec</a>.</strong> All rights reserved.
...
<!-- fin Modal -->
    ...
    <script src="../utilitarios/summernote/summernote-lite.min.js"></script>
 ...
 ***********************************************************************/
        ...
        $(document).on('shown.bs.modal', '.modal', function() {
                ...
                // 'this' se refiere al modal que se acaba de abrir.
                var modal = $(this);

                // Buscamos todos los selects con la clase .js-example-basic-single DENTRO de ese modal.
                modal.find('.js-example-basic-single').each(function () {
                    
                    // Evitamos reinicializar un select que ya lo está.
                    if (!$(this).data('select2')) {
                        $(this).select2({
                            // Asignamos el modal actual como el contenedor del dropdown.
